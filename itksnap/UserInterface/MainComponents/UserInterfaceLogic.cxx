...
// The following is needed for the title bar icon (WIN32 only)
#ifdef WIN32
  #include <windows.h>
  #include <FL/x.H>
  #include "resource.h"
...
//  m_Activation->AddWidget(m_BtnSNAPMode, UIF_GRAY_LOADED);
  ...
  for(i = 0; i < 3; i++)
    ...
    m_Activation->AddWidget(m_InSliceIndex[i], UIF_BASEIMG_LOADED);
    }
...
: UserInterface()
{
  ...
  m_IRISWindowManager3D = new RenderWindow3D( m_vtkFlRenderWindowInteractor );
  m_SliceWindow[3]->resize(0,0,0,0);
  ...
  /** Sabre grid IO dialog */
  m_DlgSabreGridIO = new SimpleFileDialogLogic();
  m_DlgSabreGridIO->MakeWindow();
  m_DlgSabreGridIO->SetFileBoxTitle("Sabre grid file:");
  m_DlgSabreGridIO->SetPattern("All text files\t*.{txt}");
  m_DlgSabreGridIO->SetLoadCallback(this,&UserInterfaceLogic::OnLoadSabreGridAction);
  m_DlgSabreGridIO->SetSaveCallback(this,&UserInterfaceLogic::OnSaveSabreGridAction);

  /** Sabre tracing IO dialog */
  m_DlgSabreTraceIO = new SimpleFileDialogLogic();
  m_DlgSabreTraceIO->MakeWindow();
  m_DlgSabreTraceIO->SetFileBoxTitle("Sabre object map:");
  m_DlgSabreTraceIO->SetPattern("All object files\t*.{obj}");
  m_DlgSabreTraceIO->SetLoadCallback(this,&UserInterfaceLogic::OnLoadSabreTraceAction);
  m_DlgSabreTraceIO->SetSaveCallback(this,&UserInterfaceLogic::OnSaveSabreTraceAction);

  /** Flood fill limit IO dialog */
  m_DlgFloodFillLimitsIO = new SimpleFileDialogLogic();
  m_DlgFloodFillLimitsIO->MakeWindow();
  m_DlgFloodFillLimitsIO->SetFileBoxTitle("Limit image:");
  m_DlgFloodFillLimitsIO->SetPattern("All image files\t*.{img}");
  m_DlgFloodFillLimitsIO->SetLoadCallback(this,&UserInterfaceLogic::OnFloodFillLoadLimitsAction);
  m_DlgFloodFillLimitsIO->SetSaveCallback(this,&UserInterfaceLogic::OnFloodFillSaveLimitsAction);


  /** ACPC alignmnet IO dialog */
  m_DlgAlignmentIO = new SimpleFileDialogLogic();
  m_DlgAlignmentIO->MakeWindow();
  m_DlgAlignmentIO->SetFileBoxTitle("Alignment transform file:");
  m_DlgAlignmentIO->SetPattern("All image files\t*.{mat}");
  m_DlgAlignmentIO->SetLoadCallback(this,&UserInterfaceLogic::OnAlignmentLoadTransformAction);
  m_DlgAlignmentIO->SetSaveCallback(this,&UserInterfaceLogic::OnAlignmentSaveTransformAction);
  ...
  m_UserLoadedSegmentation = false;

  m_ACPCAlignmentTool = new ACPCAlignmentTool();
  m_ACPCAlignmentToolIsInitialized = false;
  IsSabre3DVolumeLoaded = false;

  m_IsVolumeWindowMaximized = false;
}
...
::~UserInterfaceLogic() 
{
  ...
  delete m_DlgSabreGridIO;
  delete m_DlgSabreTraceIO;
  delete m_DlgAlignmentIO;
  delete m_DlgFloodFillLimitsIO;
...
if(ev == FL_SHORTCUT)
  ...
  if(m_Activation->GetFlag(UIF_IRIS_WITH_BASEIMG_LOADED))
    ...
    // if SABRE3D_MODE
    if ( m_GlobalState->GetToolbarMode() == SABRE3D_MODE )
    {
     if ( Fl::test_shortcut('g'))
      {
        OnBtnSabre3DTransformFunctionGuess();
        return 1;
      }
     else if ( Fl::test_shortcut('r') )
      {
        OnBtnSabre3DResetCamera();
        return 1;
      }
     else if ( Fl::test_shortcut('f') )
      {
        OnBtnSabre3DFixCamera();
        return 1;
      }
      else if ( Fl::test_shortcut('l'))
      {
        if ( m_Sabre3DInteractionLandmarkToggleButton->value() == 0 )
          m_Sabre3DInteractionLandmarkToggleButton->value( 1 );
        else
          m_Sabre3DInteractionLandmarkToggleButton->value( 0 );
        
        m_Sabre3DInteractionLandmarkToggleButton->redraw();

        OnSabre3DInteractionModeToggle();
        return 1;
      }
      ...
      {
        this->OnBtnSabre3DToggleTrace();
        return 1;
      }
    } // if SABRE3D_MODE


    if(Fl::test_shortcut('a'))
      {
      ...
      {
      if ( m_GlobalState->GetToolbarMode() == SABRE3D_MODE )
        this->OnBtnSabre3DUndo();
      else if(m_BtnIRISUndo->active())
        this->OnUndoAction();
      return 1;
      }
    ...
    else if( ( Fl::test_shortcut(',') || Fl::test_shortcut('<') ) && m_GlobalState->GetToolbarMode() != SABRE_MODE && m_GlobalState->GetToolbarMode() != SABRE3D_MODE  )
      {
    ...
    else if(Fl::test_shortcut((FL_COMMAND | ',') || Fl::test_shortcut(FL_COMMAND | '<') ) && m_GlobalState->GetToolbarMode() != SABRE_MODE && m_GlobalState->GetToolbarMode() != SABRE3D_MODE )
      {
...
::Launch()
{
  ...
  Fl::add_timeout(0.03, &UserInterfaceLogic::GlobalIdleHandler);
   ...
   m_WinMain->icon((char *)LoadIcon(fl_display, MAKEINTRESOURCE(IDI_ICON1)));
  #endif
...
::UpdateColorLabelSelection()
{
  ...
  m_InDrawingColor->value( iDrawing );
  ...
  if(m_GlobalState->GetCoverageMode() == PAINT_OVER_ALL)
    ...
    m_InDrawOverColor->value( iDrawOver + 2 );
...
::UpdateImageProbe() 
{
  ...
  // Get the segmentation label intensity
...
void 
UserInterfaceLogic
::OnSliceInputAction(int id)
{
  // Get the new value depending on the current state
  unsigned int value = m_InSliceIndex[id]->value();

  // Ensure that the new value is a valid slice index
  int top = m_InSliceSlider[id]->minimum();
  int total = - top;
  ...
  // Update the slice slider with the new value
  m_InSliceSlider[id]->value( 1-value, m_InSliceSlider[id]->slider_size(), top, total );

  OnSliceSliderChange(id);
}
...
::UpdatePositionDisplay(int id) 
{
  ...
  m_InSliceIndex[0]->value( cursor[2] + 1 );
  m_InSliceIndex[1]->value( cursor[0] + 1 );
  m_InSliceIndex[2]->value( cursor[1] + 1 );
}
...
void 
UserInterfaceLogic
::OnLineAttributesUpdate()
{
  for(unsigned int i=0;i<3;i++)
  {
    m_IRISWindowManager2D[i]->GetPolygonDrawing()->SetClosed( this->m_InIRISOpenPolygon->value() );
  }   
}
...
::OnWindowFocus(int iWindow)
{
  ...
  if ( iWindow == 3 )
    if ( m_IsVolumeWindowMaximized )
      m_IsVolumeWindowMaximized = false;
    else
      m_IsVolumeWindowMaximized = true;
...
{
  static bool needToUpdateUIAfterLandmarking = false;
  static bool needToUpdateUIAfterAlignment   = false;
  static bool needToUpdateUIAfterFloodFill   = false;
  ...
  for(unsigned int i=0;i<3;i++)
    ...
    switch(mode) 
      ...
      case POLYGON_DRAWING_MODE:
        ...
        m_IRISWindowManager2D[i]->GetPolygonDrawing()->SetClosed( true );
        m_IRISWindowManager2D[i]->GetPolygonDrawing()->SetFilled( true );
        //this->m_InIRISOpenPolygon->value( true );
      ...
      case LINE_DRAWING_MODE:
        m_IRISWindowManager2D[i]->EnterPolygonMode();
        m_IRISWindowManager2D[i]->GetPolygonDrawing()->SetClosed( this->m_InIRISOpenPolygon->value() );
        m_IRISWindowManager2D[i]->GetPolygonDrawing()->SetFilled( false );
        m_TabsToolOptions->value(m_GrpToolOptionLine);
        break;

      case FLOODFILL_MODE:
        m_GlobalState->GetFloodFillLimitImage()->Initialize( m_Driver->GetIRISImageData()->GetGrey()->GetImage() );
        m_IRISWindowManager2D[i]->EnterFloodFillMode();
        m_TabsToolOptions->value(m_GrpToolOptionFloodFill);
        break;

      case SABRE_MODE:
        m_TabsToolOptions->value(m_GrpToolOptionSABRE1);
        m_IRISWindowManager2D[i]->EnterCrosshairsMode();
        break;

      case SABRE3D_MODE:
        m_TabsToolOptions->value(m_GrpToolOptionSABRE3D);
        m_IRISWindowManager2D[i]->EnterCrosshairsMode();
        break;

      case ALIGNMENT_MODE:
        m_TabsToolOptions->value(m_GrpToolOptionAlignment);
        m_IRISWindowManager2D[i]->EnterAlignmentMode();
        break;
      ...
      if (mode == POLYGON_DRAWING_MODE || mode == LINE_DRAWING_MODE)
        m_GrpPolygonEdit[i]->show();
      else
        m_GrpPolygonEdit[i]->hide();

      if ( mode == FLOODFILL_MODE )
        m_GrpFloodFillEdit[i]->show();
      else
        m_GrpFloodFillEdit[i]->hide();

      if ( mode == SABRE_MODE || mode == ALIGNMENT_MODE || mode == SABRE3D_MODE )
      {
        m_GrpSegmentationOptions->hide();
        m_Grp3DToolbox->hide();
      }
      else
      {
        m_GrpSegmentationOptions->show();
        m_Grp3DToolbox->show();
      }
    }

  // If flood filling complete, update UI and unset restore needed flag
  if ( needToUpdateUIAfterFloodFill == true )
  {
    RestoreUIAfterFloodFill();
    UpdatePaintbrushAttributes();
    OnIRISLabelOpacityChange();
    OnDrawingLabelUpdate();
    OnDrawOverLabelUpdate();
    OnLabelListUpdate(); 
    needToUpdateUIAfterFloodFill = false;
  }
  //  If landmarking complete, update UI and unset restore needed flag
  if ( needToUpdateUIAfterLandmarking == true )
  {
   RestoreUIAfterLandmarking();
   needToUpdateUIAfterLandmarking = false;
  }
  // If alignment complete, update UI and unset restore needed flag
  if ( needToUpdateUIAfterAlignment == true )
  {
    RestoreUIAfterAlignment();
    needToUpdateUIAfterAlignment = false;
    m_Driver->GetIRISImageData()->GetGrey()->SetImage( m_ACPCAlignmentTool->GetOriginalImage() );
  }

  // If starting flood fill, update UI and set restore need flag
  if ( mode == FLOODFILL_MODE )
  {
    SetupUIBeforeFloodFill();
    needToUpdateUIAfterFloodFill = true; 
  }
  // If starting landmarking, update UI and set restore neeeded flag
  if ( mode == SABRE_MODE )
  {
    SetupUIBeforeLandmarking();
    needToUpdateUIAfterLandmarking = true;
  }
  // If starting alignment, update UI and set restore needed flag
  if ( mode == ALIGNMENT_MODE )
  {
    SetupUIBeforeAlignment();
    InitializeAlignmentTool();
    OnBtnAlignmentChange(0,0,0,0,0,0);
    needToUpdateUIAfterAlignment = true;
  }

  if ( mode == PAINTBRUSH_MODE || mode == FLOODFILL_MODE ) 
  {
    m_ChkInvertPolygon->deactivate();
  }
  else
  {
    m_ChkInvertPolygon->activate();
  }

  
  if ( m_GlobalState->GetToolbarMode()  == SABRE3D_MODE )
    m_Sabre3DToolbox->show();
  else
    m_Sabre3DToolbox->hide();
...
void 
UserInterfaceLogic
::SetLandmarkMode( LandmarkType landmark, ToolbarModeType tool, int paintbrushSize )
{
  m_GlobalState->SetDrawingColorLabel( (LabelType)(size_t)( landmark ) );
  m_GlobalState->GetLandmarks()->SetCurrentLandmarkMode( landmark );

  if ( tool == LANDMARK_PAINTBRUSH_MODE || tool == PAINTBRUSH_MODE )
  {
    // Paintbrush mode for AC, PC, PE, CA, M, LPRON, RPRON, RSC and LSC
    PaintbrushSettings pbs = m_GlobalState->GetPaintbrushSettings();
    pbs.radius = 0.5 * paintbrushSize;
    ...
    for(unsigned int i = 0; i < 3; i++)
    {
       m_IRISWindowManager2D[i]->EnterLandmarkPaintbrushMode();
       m_GrpPolygonEdit[i]->hide();
    }
  }

  if ( tool == LANDMARK_LINE_DRAWING_MODE )
  {
    // Line mode for LOP and ROP
    for(unsigned int i = 0; i < 3; i++)
    {
       m_IRISWindowManager2D[i]->EnterPolygonMode();
       m_IRISWindowManager2D[i]->GetPolygonDrawing()->SetClosed( this->m_InIRISOpenPolygon->value() );
       m_IRISWindowManager2D[i]->GetPolygonDrawing()->SetFilled( false );
       m_GlobalState->SetPolygonInvert( false );
       m_GrpPolygonEdit[i]->show();
    }
  }

  if ( tool == CROSSHAIRS_MODE )
  {
    // Crosshairs mode for when no landmark is selected
    for(unsigned int i = 0; i < 3; i++)
    {
       m_IRISWindowManager2D[i]->EnterCrosshairsMode();
       m_GrpPolygonEdit[i]->hide();
...
void 
UserInterfaceLogic
::UpdateSabreGridDisplay()
{
  m_OutAC->value( m_GlobalState->GetLandmarks()->GetAC() );
  m_OutPC->value( m_GlobalState->GetLandmarks()->GetPC() );
  m_OutPE->value( m_GlobalState->GetLandmarks()->GetPE() );
  m_OutCA->value( m_GlobalState->GetLandmarks()->GetCA() );
  m_OutM->value(  m_GlobalState->GetLandmarks()->GetM()  );
  m_OutLPRON->value( m_GlobalState->GetLandmarks()->GetLPRON() );
  m_OutRPRON->value( m_GlobalState->GetLandmarks()->GetRPRON() ); 

  std::ostringstream ss;
  ss << "Axial ACPC Slice: " << m_GlobalState->GetLandmarks()->GetAP();
  m_AxialPlaneForACPC->copy_label( ss.str().c_str() );

  ss.str("");
  ss << "Sagittal Slice: " << m_GlobalState->GetLandmarks()->GetCA();
  m_SagittalPlaneForM->copy_label( ss.str().c_str() );

  ss.str("");
  ss << "Coronal Slice: " << m_GlobalState->GetLandmarks()->GetPON();
  m_CoronalPlaneForPON->copy_label( ss.str().c_str() );

  int rightSagPlane = m_GlobalState->GetLandmarks()->GetM();
  if ( rightSagPlane != 0 ) 
    rightSagPlane -= 7;

  ss.str("");
  ss << "Right Sagittal Slice: " << rightSagPlane;
  m_RSagittalPlaneForLineTracing->copy_label( ss.str().c_str() );
  
  int leftSagPlane = m_GlobalState->GetLandmarks()->GetM();
  if ( leftSagPlane != 0 ) 
    leftSagPlane += 7;

  ss.str("");
  ss << "Left Sagittal Slice: " << leftSagPlane;
  m_LSagittalPlaneForLineTracing->copy_label( ss.str().c_str() );
...
{
  // Save UI state
  m_SystemInterface->AssociateCurrentSettingsWithCurrentImageFile( m_GlobalState->GetGreyFileName(), m_Driver );

  // Turn all radio buttons off
  for ( int j = 0; j < sizeof(m_RadioSabre)/sizeof(m_RadioSabre[0]); ++j )
    m_RadioSabre[j]->clear();
  
  // Configure UI for landmarking
  this->GetDriver()->GetColorLabelTable()->InitializeToDefaults();
  
  m_GlobalState->SetSegmentationAlpha( 255 );
  m_GlobalState->SetCoverageMode( PAINT_OVER_ALL );

  // Store undo point (without altering state of UIF_UNSAVED_CHANGES)
  bool unsavedChanges = m_Activation->GetFlag( UIF_UNSAVED_CHANGES );
  StoreUndoPoint("PreSabre");
  m_Activation->UpdateFlag( UIF_UNSAVED_CHANGES, unsavedChanges );
}
...
{
  // Restore UI state
  m_SystemInterface->RestoreSettingsAssociatedWithImageFile( m_GlobalState->GetGreyFileName(), m_Driver, true, true, true, true );
...
::OnBtnLoadSabreGrid()
{
 ...
 if( !PromptBeforeLosingChanges( REASON_LOAD_SABRE_GRID ) ) return;
  
 // Guess at the filename
 std::string fn( m_GlobalState->GetGreyFileName() );
 int pos = fn.rfind(".");
 if ( pos != fn.npos )
 {
    fn.erase(pos);
  ...
  // Display simple load dialog
  m_DlgSabreGridIO->SetTitle( "Load SABRE grid file:" );
  m_DlgSabreGridIO->DisplayLoadDialog( m_SystemInterface->GetHistory("SabreGrid"), fn.c_str() );
}

void
UserInterfaceLogic
::OnLoadSabreGridAction()
{
  const char *file = m_DlgSabreGridIO->GetFileName();

  m_GlobalState->GetLandmarks()->RemoveGridLandmarks( m_Driver->GetIRISImageData()->GetSegmentation() );

  // Try reading from file
  try 
  {
    m_GlobalState->GetLandmarks()->LoadSabreGrid( file );
  }
  catch (itk::ExceptionObject &exc) 
  {
    // Alert the user to the failure
    fl_alert("Error reading grid file: \n%s",exc.GetDescription());
    return;
  }
  ...
  this->SetLandmarkMode( CLEAR, CROSSHAIRS_MODE );
  
  // Update UI with new grid cooridinates
  UpdateSabreGridDisplay();

  // Update history
  m_SystemInterface->UpdateHistory("SabreGrid", file);
}
...
::OnBtnSaveSabreGrid()
{
 ...
 // Display simple save dialog
  m_DlgSabreGridIO->SetTitle( "Save SABRE grid file:" );
  m_DlgSabreGridIO->DisplaySaveDialog( m_SystemInterface->GetHistory("SabreGrid"), fn.c_str() );
}

void 
UserInterfaceLogic
::OnSaveSabreGridAction()
{
  const char *file = m_DlgSabreGridIO->GetFileName();
  ...
  try 
  {
    m_GlobalState->GetLandmarks()->SaveSabreGrid( file );
  }
  catch (itk::ExceptionObject &exc) 
  {
    // Alert the user to the failure
    fl_alert("Error saving grid file: \n%s",exc.GetDescription());
    return;
  }

  // Remove landmarks from segmentation image
  m_GlobalState->GetLandmarks()->RemoveGridLandmarks(  m_Driver->GetIRISImageData()->GetSegmentation() );
...
::OnBtnLoadSabreTrace()
{
  if ( ( GetDriver()->GetGlobalState()->GetToolbarMode() == SABRE3D_MODE ) && !IsSabre3DVolumeLoaded )
  {
    fl_alert("Before loading an object map, initialize the renderer by pressing one of the 3D viewpoint buttons");
     return;
  }
  
  if ( IsSabre3DVolumeLoaded )
  {
    OnBtnSabre3DDeleteLandmarks();
  }
 ...
 // Prompt user to save changes
 if( !PromptBeforeLosingChanges( REASON_LOAD_SABRE_TRACE ) ) return;
 ...
 if ( pos != fn.npos )
 {
  ...
  // Display simple load dialog
  m_DlgSabreTraceIO->SetTitle( "Load SABRE tracing object map:" );
  m_DlgSabreTraceIO->DisplayLoadDialog( m_SystemInterface->GetHistory(""), fn.c_str() );
}

void
UserInterfaceLogic
::OnLoadSabreTraceAction()
{
  const char* file = m_DlgSabreTraceIO->GetFileName();

  try 
  {
    m_GlobalState->GetLandmarks()->LoadSabreTrace( file, m_Driver->GetIRISImageData()->GetSegmentation() );
  }
  catch ( itk::ExceptionObject &exc ) 
  {
    // Alert the user to the failure
    fl_alert("Error loading object map tracing: \n%s",exc.GetDescription());
  }
 ...
 if ( m_GlobalState->GetToolbarMode() == SABRE3D_MODE )
 {
   m_IRISWindowManager3D->GetInteractor()->OnLandmarkLoad( m_Driver->GetIRISImageData()->GetSegmentation(), m_Driver->GetIRISImageData()->GetGrey() );
 }
}
...
::OnBtnSaveSabreTrace()
{
 ...
 if ( pos != fn.npos )
 {
  ...
  // Display simple save dialog
  m_DlgSabreTraceIO->SetTitle( "Save SABRE tracing as object map:" );
  m_DlgSabreTraceIO->DisplaySaveDialog( m_SystemInterface->GetHistory(""), fn.c_str() );
}

void 
UserInterfaceLogic
::OnSaveSabreTraceAction()
{
  const char *file = m_DlgSabreTraceIO->GetFileName();
  m_GlobalState->GetLandmarks()->SaveSabreTrace( file, m_Driver->GetIRISImageData()->GetSegmentation() );
...
{
  // Prompt user to save changes
  if( !PromptBeforeLosingChanges( REASON_RESET_SABRE ) ) return;

  // Clear landmarks
  m_GlobalState->GetLandmarks()->ClearAllLandmarks();
  UpdateSabreGridDisplay();
  
  // Clear segmentation image
  m_Driver->ClearIRISSegmentationImage();
  OnSegmentationImageUpdate(true);
...
::InitializeSabre3D()
{
  if ( !IsSabre3DVolumeLoaded )
  {
    m_IRISWindowManager3D->SetImage( m_Driver->GetIRISImageData()->GetGrey()->GetImage() );
    IsSabre3DVolumeLoaded = true;
  
  m_Sabre3DTraceToggleButton->label( "hide");
  RenderWindow3D::TransferFunctionParameters param = m_IRISWindowManager3D->GetDefaultTransformFunction();
  
  m_BtnTransferFunctionColor1[0]->value( param.x1 );
  m_BtnTransferFunctionColor2[0]->value( param.x2 );
  m_BtnTransferFunctionScalarOpacity[0]->value( param.x3 );
  
  m_BtnTransferFunctionColor1[1]->value( param.y1 );
  m_BtnTransferFunctionColor2[1]->value( param.y2 );
  m_BtnTransferFunctionScalarOpacity[1]->value( param.y3 );
  
  m_BtnTransferFunctionColor1[2]->value( param.z1 );
  m_BtnTransferFunctionColor2[2]->value( param.z2 );
  m_BtnTransferFunctionScalarOpacity[2]->value( param.z3 );
  m_SliderSabre3D->value( param.z3 );
  
  m_RadioSabre3D[0]->value( 1 );
  for ( int i = 1; i <= 3; ++i )
    m_RadioSabre3D[i]->value( 0 );
...
::OnSliderSabre3D()
{
 double value = m_SliderSabre3D->value();
 RenderWindow3D::TransferFunctionParameters param = m_IRISWindowManager3D->GetDefaultTransformFunction();
 m_BtnTransferFunctionScalarOpacity[1]->value( value );
 param.z3 = value;
 m_IRISWindowManager3D->AdjustTransferFunction( param.x1, param.x2, param.x3, param.y1, param.y2, param.y3, param.z1, param.z2, param.z3 );
 m_vtkFlRenderWindowInteractor->redraw();
}
...
{
  InitializeSabre3D();
  m_IRISWindowManager3D->RenderRight();   
}
...
{
  InitializeSabre3D();
  m_IRISWindowManager3D->RenderLeft(); 
}
...
{
  InitializeSabre3D();
  m_IRISWindowManager3D->RenderTop();
}
...
{
  InitializeSabre3D();
  m_IRISWindowManager3D->RenderBottom();
}
...
{
  InitializeSabre3D();
  m_IRISWindowManager3D->RenderFront();
}
...
{
  InitializeSabre3D();
  m_IRISWindowManager3D->RenderBack();
}
...
::OnSabre3DInteractionModeToggle()
{
  m_IRISWindowManager3D->ToggleInteractionMode();
}

void
UserInterfaceLogic
::Set3DLandmarkMode( LandmarkType landmark )
{
  m_IRISWindowManager3D->GetInteractor()->SetLandmarkMode( landmark );
}

void 
UserInterfaceLogic
::OnSabre3DTransferFunctionAction( unsigned int button )
{
  m_IRISWindowManager3D->AdjustTransferFunction( 
    m_BtnTransferFunctionColor1[ 0 ]->value(), m_BtnTransferFunctionColor2[ 0 ]->value(), m_BtnTransferFunctionScalarOpacity[ 0 ]->value(), 
    m_BtnTransferFunctionColor1[ 1 ]->value(), m_BtnTransferFunctionColor2[ 1 ]->value(), m_BtnTransferFunctionScalarOpacity[ 1 ]->value(),
    m_BtnTransferFunctionColor1[ 2 ]->value(), m_BtnTransferFunctionColor2[ 2 ]->value(), m_BtnTransferFunctionScalarOpacity[ 2 ]->value()
  );
  m_vtkFlRenderWindowInteractor->redraw();
}
...
::OnBtnSabre3DAccept()
{
  m_IRISWindowManager3D->GetInteractor()->Accept( m_Driver->GetIRISImageData()->GetSegmentation() );
}
...
::OnBtnSabre3DUndo()
{
  m_IRISWindowManager3D->GetInteractor()->Undo();
}
...
::OnBtnSabre3DRedo()
{
  m_IRISWindowManager3D->GetInteractor()->Redo();
}

void
UserInterfaceLogic
::OnBtnSabre3DDeleteLandmark()
{
  if ( m_GlobalState->GetToolbarMode() == SABRE3D_MODE )
  {
    if ( m_RadioSabre3D[0]->value() == 1 )
      m_IRISWindowManager3D->GetInteractor()->DeleteLandmark( m_Driver->GetIRISImageData()->GetSegmentation(), LSF );
    else if ( m_RadioSabre3D[1]->value() == 1 )
      m_IRISWindowManager3D->GetInteractor()->DeleteLandmark( m_Driver->GetIRISImageData()->GetSegmentation(), RSF );
    else if ( m_RadioSabre3D[2]->value() == 1 )
      m_IRISWindowManager3D->GetInteractor()->DeleteLandmark( m_Driver->GetIRISImageData()->GetSegmentation(), LC );
    else if ( m_RadioSabre3D[3]->value() == 1 )
      m_IRISWindowManager3D->GetInteractor()->DeleteLandmark( m_Driver->GetIRISImageData()->GetSegmentation(), RC );
  }
  OnBtnSabre3DDisconnect();
}
...
::OnBtnSabre3DDisconnect()
{
  m_IRISWindowManager3D->GetInteractor()->Disconnect();
}
...
void
UserInterfaceLogic
::OnBtnSabre3DResetModule()
{
  m_IRISWindowManager3D->ResetWindow();
  IsSabre3DVolumeLoaded = false;
  if ( m_Driver->GetIRISImageData()->IsSegmentationLoaded() )
    m_Driver->ClearIRISSegmentationImage();
}
...
::OnBtnSabre3DFixCamera()
{
  m_IRISWindowManager3D->FixCamera();
}
...
::OnBtnSabre3DTransformFunctionGuess()
{
  RenderWindow3D::TransferFunctionParameters guess;
  guess = m_IRISWindowManager3D->GuessAtTransformFunction(  m_Driver->GetIRISImageData()->GetGrey()->GetImage()  );
  
  m_BtnTransferFunctionColor1[0]->value( guess.x1 );
  m_BtnTransferFunctionColor1[1]->value( guess.y1 );
  m_BtnTransferFunctionColor1[2]->value( guess.z1 );
  
  m_BtnTransferFunctionColor2[0]->value( guess.x2 );
  m_BtnTransferFunctionColor2[1]->value( guess.y2 );
  m_BtnTransferFunctionColor2[2]->value( guess.z2 );

  m_BtnTransferFunctionScalarOpacity[0]->value( guess.x3 );
  m_BtnTransferFunctionScalarOpacity[1]->value( guess.y3 );
  m_BtnTransferFunctionScalarOpacity[2]->value( guess.z3 );

  m_SliderSabre3D->value( guess.z3 );
}
...
::OnBtnSabre3DResetCamera()
{
  m_IRISWindowManager3D->ResetCamera();
}


void
UserInterfaceLogic
::OnBtnSabre3DToggleTrace()
{
  if ( m_Sabre3DTraceToggleButton->label() == "show" )
  {
    m_Sabre3DTraceToggleButton->label( "hide" );
    m_IRISWindowManager3D->GetInteractor()->Unhide();
  }
  else
  {
    m_Sabre3DTraceToggleButton->label( "show" );
    m_IRISWindowManager3D->GetInteractor()->Hide();
  }
}
...
void 
UserInterfaceLogic
::OnFloodFillClearLimitsAction()
{
  m_GlobalState->GetFloodFillLimitImage()->ClearAllLimits();
}

void
UserInterfaceLogic
::OnFloodFillClearCurrentSliceLimits( unsigned int window )
{
  ...
  m_GlobalState->GetFloodFillLimitImage()->ClearLimitsOnSlice( plane, m_Driver->GetCursorPosition() );
  RedrawWindows();
}

void
UserInterfaceLogic
::OnFloodFillRedoLimit( unsigned int window )
{
  unsigned int plane = 0;
  if ( window == 0 )
    plane = 2;
  ...
  m_GlobalState->GetFloodFillLimitImage()->RedoLastLimit( plane, m_Driver->GetCursorPosition() );
  RedrawWindows();
}

void
UserInterfaceLogic
::OnFloodFillUndoLimit( unsigned int window )
{
  unsigned int plane = 0;
  if ( window == 0 )
    plane = 2;
  ...
  m_GlobalState->GetFloodFillLimitImage()->UndoLastLimit( plane, m_Driver->GetCursorPosition() );
  RedrawWindows();
}

void
UserInterfaceLogic
::OnBtnFloodFillLoadLimits()
{
 // Prompt user to save changes
 // if( !PromptBeforeLosingChanges( REASON_LOAD_SABRE_TRACE ) ) return;
 ...
 if ( pos != fn.npos )
 {
  ...
  // Display simple load dialog
  m_DlgFloodFillLimitsIO->SetTitle( "Load limits image for flood fill:" );
  m_DlgFloodFillLimitsIO->DisplayLoadDialog( m_SystemInterface->GetHistory(""), fn.c_str() );
}


void
UserInterfaceLogic
::OnFloodFillLoadLimitsAction()
{
  const char *file = m_DlgFloodFillLimitsIO->GetFileName();
  
  try 
  {
    m_GlobalState->GetFloodFillLimitImage()->LoadLimitImage( file );
  }
  catch ( itk::ExceptionObject &exc ) 
  {
    // Alert the user to the failure
    fl_alert("Error loading limit image: \n%s",exc.GetDescription());
  }
}

void 
UserInterfaceLogic
::OnBtnFloodFillSaveLimits()
{
   // Guess at the filename
 std::string fn( m_GlobalState->GetGreyFileName() );
 int pos = fn.rfind(".");
 if ( pos != fn.npos )
 {
  ...
  // Display simple save dialog
  m_DlgFloodFillLimitsIO->SetTitle( "Save limit image for flood fill:" );
  m_DlgFloodFillLimitsIO->DisplaySaveDialog( m_SystemInterface->GetHistory(""), fn.c_str() );
}


void 
UserInterfaceLogic
::OnFloodFillSaveLimitsAction()
{
  const char *file = m_DlgFloodFillLimitsIO->GetFileName();
  try 
  {
    m_GlobalState->GetFloodFillLimitImage()->SaveLimitImage( file );
  }
  catch ( itk::ExceptionObject &exc ) 
  {
    // Alert the user to the failure
    fl_alert("Error saving limit image: \n%s",exc.GetDescription());
  } 
}
...
::RestoreUIAfterFloodFill()
{
  ...
  SNAPAppearanceSettings::Element &elt = GetAppearanceSettings()->GetUIElement(SNAPAppearanceSettings::PAINTBRUSH_OUTLINE);
...
::OnBtnFloodFill3D()
{
  for ( int i=0; i<3; ++i )
  {
    m_IRISWindowManager2D[i]->SetFloodFill3D( m_BtnFloodFill3D->value() );
  }
}

void
UserInterfaceLogic
::OnBtnAlignmentChange( int degreesX, int degreesY, int degreesZ, int shiftX, int shiftY, int shiftZ )
{
  GreyImageWrapper::ImagePointer original = m_Driver->GetIRISImageData()->GetGrey()->GetImage();
  Vector3ui cursor = m_Driver->GetCursorPosition();
  try 
  {
    m_ACPCAlignmentTool->AlignmentChange( degreesX, degreesY, degreesZ, shiftX, shiftY, shiftZ, cursor, original );
  }
  catch ( itk::ExceptionObject &exc )
  {
    fl_alert( "Error executing ACPC alignment change: \n%s", exc.GetDescription() );
    return;
  }

  m_Driver->GetIRISImageData()->GetGrey()->SetImage( original );
  RedrawWindows();
...
::UpdateAlignmentDisplay()
{
  vnl_matrix< double > transform = m_ACPCAlignmentTool->GetAnalyzeTransform();
  for ( int i=0; i < 12; ++i )
    m_InRotationMatrix[i]->precision(3);

  m_InRotationMatrix[0]->value(transform[0][0]);
  m_InRotationMatrix[1]->value(transform[0][1]);
  m_InRotationMatrix[2]->value(transform[0][2]);
  m_InRotationMatrix[3]->value(transform[1][0]);
  m_InRotationMatrix[4]->value(transform[1][1]);
  m_InRotationMatrix[5]->value(transform[1][2]);
  m_InRotationMatrix[6]->value(transform[2][0]);
  m_InRotationMatrix[7]->value(transform[2][1]);
  m_InRotationMatrix[8]->value(transform[2][2]);
  
  m_InRotationMatrix[9]->value(transform[3][0]);
  m_InRotationMatrix[10]->value(transform[3][1]);
  m_InRotationMatrix[11]->value(transform[3][2]);
}

void
UserInterfaceLogic
::OnBtnAlignmentInverseTransform()
{
  m_ACPCAlignmentTool->SetInverseTransform();
  OnBtnAlignmentChange(0,0,0,0,0,0);
  UpdateAlignmentDisplay();
}
...
void
UserInterfaceLogic
::OnBtnAlignmentIdentityTransform()
{
  m_ACPCAlignmentTool->SetIdentityTransform();
  OnBtnAlignmentChange(0,0,0,0,0,0);
  UpdateAlignmentDisplay();  
}
...
void 
UserInterfaceLogic
::OnBtnAlignmentResample()
{
  try
  {
    m_ACPCAlignmentTool->Resample();
  }
  catch ( itk::ExceptionObject &exc )
  {
    fl_alert( "Error resampling image: \n%s", exc.GetDescription() );
    return;
  }
  OnBtnAlignmentChange(0,0,0,0,0,0);
}
...
::OnBtnAlignmentLoadTransform()
{
 ...
 if ( pos != fn.npos )
 {
  ...
  // Display simple load dialog
  m_DlgAlignmentIO->SetTitle( "Load alignment transform:" );
  m_DlgAlignmentIO->DisplayLoadDialog( m_SystemInterface->GetHistory(""), fn.c_str() );
}

void
UserInterfaceLogic
::OnAlignmentLoadTransformAction()
{
  const char *file = m_DlgAlignmentIO->GetFileName();
  try 
  {
    m_ACPCAlignmentTool->LoadTransform( file );
  }
  catch ( itk::ExceptionObject &exc )
  {
    fl_alert( "Error loading transform: \n%s", exc.GetDescription() );
    return;
  }
  OnBtnAlignmentChange(0,0,0,0,0,0);
  RedrawWindows();
...
::OnBtnAlignmentSaveTransform()
{
 ...
 if ( pos != fn.npos )
 {
  ...
  // Display simple save dialog
  m_DlgAlignmentIO->SetTitle( "Save alignment transform:" );
  m_DlgAlignmentIO->DisplaySaveDialog( m_SystemInterface->GetHistory(""), fn.c_str() );

}

void 
UserInterfaceLogic
::OnAlignmentSaveTransformAction()
{
 const char *file = m_DlgAlignmentIO->GetFileName();
 try 
  {
    m_ACPCAlignmentTool->SaveTransform( file );
  }
  catch ( itk::ExceptionObject &exc )
  {
    fl_alert( "Error saving transform: \n%s", exc.GetDescription() );
    return;
  }
}
...
void 
UserInterfaceLogic
::OnAlignmentInterpolateAction()
{
  m_ACPCAlignmentTool->SetInterpolate( m_BtnAlignmentInterpolate->value() );
  RedrawWindows();
}
...
::RestoreUIAfterAlignment()
{
  for ( int i=0; i<3; ++i )
  {
    m_InSliceSlider[i]->activate();
    m_InSliceIndex[i]->activate();
  }
}
...
::SetupUIBeforeAlignment()
{
  ...
  for ( int i=0; i<3; ++i )
  {
    m_InSliceSlider[i]->deactivate();
    m_InSliceIndex[i]->deactivate();
  }
}
...
::InitializeAlignmentTool()
{
  try
  {
    m_ACPCAlignmentTool->Initialize(  m_Driver->GetIRISImageData()->GetGrey()->GetImage() );
  }
  catch ( itk::ExceptionObject &exc )
  {
    fl_alert( "Error initializing ACPC alignment tool:\n", exc.GetDescription() );
  }
  for ( int i = 0; i < 3; ++i )
  {
    // todo:  remove this call
     m_IRISWindowManager2D[i]->GetAlignmentInteractionMode()->SetACPCAlignmentTool( m_ACPCAlignmentTool );
  }
}
...
::OnGreyImageUnload()
{
  ...
  if(fnGrey.length())
    m_SystemInterface->AssociateCurrentSettingsWithCurrentImageFile(fnGrey.c_str(),m_Driver);
...
::UnloadAllImages()
{
  // Reset 3D volume render window
  OnBtnSabre3DDeleteLandmarks();
  OnBtnSabre3DResetModule();
  
  // Deallocate limit image (flood fill tool)
  GreyImageWrapper::ImagePointer limitImage = m_GlobalState->GetFloodFillLimitImage()->GetImage();
  limitImage->ReleaseData();
  limitImage = NULL;

  // Deallocate acpc image (alignment tool)
  GreyImageWrapper::ImagePointer acpcImage = m_ACPCAlignmentTool->GetOriginalImage();
  acpcImage->ReleaseData();
  acpcImage = NULL;
...
::OnMainImageUpdate()
{
  ...
  // Will need to reinitialize the flood fill tool
  m_GlobalState->GetFloodFillLimitImage()->Reset( m_Driver->GetIRISImageData()->GetGrey()->GetImage() );
...
::PromptBeforeLosingChanges(PromptReason reason)
{
  ...
  // NOTE:  UIF_UNSAVED_CHANGES tracks changes made to the segmentation using the original ITKSNAP
  //        interactors (including the line tool used for SABRE seg landmarks LOP and ROP), however,
  //        in SABRE mode, segmentation may also be changed using the LANDMARK_PAINTBRUSH_INTERACTOR_MODE
  //        which does not update UIF_UNSAVED_CHANGES and therefore an additional check via 
  //        SabreTraceModified() is required.  Changes to the SABRE grid are accessed via SabreGridModified().

  // If doing anything but a sabre reset or grid load, the grid is unchanged, so prompt only to save changes to seg
  if ( reason == REASON_LOAD_SABRE_TRACE || reason == REASON_LOAD_MAIN || reason == REASON_LOAD_SEGMENTATION )
  {
    if ( m_GlobalState->GetLandmarks()->SabreTraceModified() || m_Activation->GetFlag( UIF_UNSAVED_CHANGES ) )
    {
      prompt = true;
      m_WizConfirmMessage->value( m_WinConfirmMessageSegmentation );   
    }
  }
  // Check for changes to grid and segmentation
  else if ( m_GlobalState->GetLandmarks()->SabreGridModified() 
       &&  ( m_GlobalState->GetLandmarks()->SabreTraceModified() || m_Activation->GetFlag( UIF_UNSAVED_CHANGES ) ) )
  {
    prompt = true;
    m_WizConfirmMessage->value( m_WinConfirmMessageSabreGridAndSegmentation );
  }
  // Check for changes to grid only
  else if ( m_GlobalState->GetLandmarks()->SabreGridModified() )
  {
    prompt = true;
    m_WizConfirmMessage->value( m_WinConfirmMessageSabreGrid );  
  }
  // Check for changes to segmentation only
  else if ( m_GlobalState->GetLandmarks()->SabreTraceModified() || m_Activation->GetFlag( UIF_UNSAVED_CHANGES ) )
  {
    prompt = true;
    m_WizConfirmMessage->value( m_WinConfirmMessageSegmentation );   
  }

  // If necessary, prompt user to save changes
  if ( prompt )
  {
...
::OnMenuLoadGrey() 
{
  ...
  // egmod 
  const SystemInterface::HistoryListType& greyHistory = m_SystemInterface->GetHistory("GreyImage");
  std::string fn;
  if ( greyHistory.size() > 0 )
    fn = greyHistory[ greyHistory.size() - 1 ];
  else
    fn = "";
  m_WizGreyIO->DisplayInputWizard( fn.c_str(), "Greyscale");
...
::OnMenuLoadGreyOverlay()
{
  ...
  // egmod
  wizGreyOverlayIO.DisplayInputWizard(GetMostRecentGreyDirectory().c_str(), "Greyscale overlay");
  // Show the input wizard
  //wizGreyOverlayIO.DisplayInputWizard(m_GlobalState->GetGreyOverlayFileName(), "Greyscale overlay");
...
::LoadSegmentation(const bool noninteractive, const char *fname)
{
  ...
  if(noninteractive)
    ...
    {
      // egmod 
      m_WizSegmentationIO->DisplayInputWizard( this->GetMostRecentGreyDirectory().c_str(), "segmentation" );
      
      // Show the input wizard
      //m_WizSegmentationIO->DisplayInputWizard(m_GlobalState->GetLastAssociatedSegmentationFileName(), "segmentation");
    }
...
::OnMenuSaveSegmentation() 
{
  ...
  // egmod 
  bool success;
  if ( m_UserLoadedSegmentation )
  {
    success = m_WizSegmentationIO->DisplaySaveWizard( m_Driver->GetIRISImageData()->GetSegmentation()->GetImage(), m_GlobalState->GetLastAssociatedSegmentationFileName(), "segmentation");
  }
  else
  {
    // egmod
    //success = m_WizSegmentationIO->DisplaySaveWizard( m_Driver->GetIRISImageData()->GetSegmentation()->GetImage(), GetMostRecentGreyDirectory().c_str(), "segmentation");
    success = m_WizSegmentationIO->DisplaySaveWizard( m_Driver->GetIRISImageData()->GetSegmentation()->GetImage(), fn.c_str() );
  }

  // Save the segmentation
  //if(m_WizSegmentationIO->DisplaySaveWizard( m_Driver->GetIRISImageData()->GetSegmentation()->GetImage(), m_GlobalState->GetLastAssociatedSegmentationFileName(), "segmentation"))
  if ( success ) 
  {
    // Update the history for the wizard
    m_SystemInterface->UpdateHistory(
...
::OnUndoAction()
{
  ...
  if ( m_GlobalState->GetToolbarMode() == SABRE_MODE ) return;
...
{
  if ( m_GlobalState->GetToolbarMode() == SABRE_MODE ) 
  {
    // line tool
...
std::string
UserInterfaceLogic
::GetMostRecentGreyDirectory()
{
   
  const SystemInterface::HistoryListType& greyHistory = m_SystemInterface->GetHistory("GreyImage");
  std::string fn = greyHistory[ greyHistory.size() - 1 ];
