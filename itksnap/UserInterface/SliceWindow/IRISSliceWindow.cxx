...
: GenericSliceWindow(id, parentUI, canvas)
{
  ...
  m_LandmarkPaintbrushMode = new LandmarkPaintbrushInteractionMode(this);
  m_FloodFillMode = new FloodFillInteractionMode(this);
  m_AlignmentMode = new AlignmentInteractionMode(this);
  ...
  m_LandmarkPaintbrushMode->Register();
  m_FloodFillMode->Register();
  m_AlignmentMode->Register();
}
...
::~IRISSliceWindow()
{
  ...
  delete m_LandmarkPaintbrushMode;
  delete m_FloodFillMode;
...
::DrawOverlays()
{
  ...
  if(IsInteractionModeAdded(m_LandmarkPaintbrushMode) && !m_ThumbnailIsDrawing)
    m_LandmarkPaintbrushMode->OnDraw();

  // Always draw annotations, whether active or not - but not in the thumbnail
  if(!m_ThumbnailIsDrawing)
    m_AnnotationMode->OnDraw();

  if(IsInteractionModeAdded(m_FloodFillMode) && !m_ThumbnailIsDrawing)
    m_FloodFillMode->OnDraw();
...
void 
IRISSliceWindow
::EnterLandmarkPaintbrushMode()
{
  EnterInteractionMode(m_LandmarkPaintbrushMode);
}
...
::EnterAlignmentMode()
{
  EnterInteractionMode(m_AlignmentMode);
}
...
::EnterFloodFillMode()
{
  EnterInteractionMode(m_FloodFillMode);
}
...
::AcceptPolygon() 
{
  ...
  if (m_GlobalState->GetDrawingLock(id)) {
    ...
    LabelImageWrapper *seg =  m_Driver->GetCurrentImageData()->GetSegmentation();
    ...
    PolygonIterator itPolygon(m_PolygonSlice, m_PolygonSlice->GetLargestPossibleRegion());

    // Keep track of the number of pixels changed
    unsigned int nUpdates = 0;

    // Iterate
    for (itPolygon.GoToBegin(); !itPolygon.IsAtEnd(); ++itPolygon)
    {
...
void 
IRISSliceWindow
::SetFloodFill3D( bool value )
{
  m_FloodFillMode->SetFill3D( value );
}
