...
::ApplyBrush(FLTKEvent const &event)
{
  ...
  for(; !it.IsAtEnd(); ++it)
    ...
    if(event.Button == FL_LEFT_MOUSE)
      {
      bool visible = this->m_Driver->GetColorLabelTable()->GetColorLabel(it.Get()).IsVisible();
      if (mode == PAINT_OVER_ALL || 
        (mode == PAINT_OVER_ONE && pxLabel == overwrt_color) ||
        (mode == PAINT_OVER_COLORS && visible )) //pxLabel != 0 ))
        {
        //if ( mode == PAINT_OVER_COLORS && !this->m_Driver->GetColorLabelTable()->GetColorLabel(it.Get()).IsVisible() ) continue;
...
Vector3f
PaintbrushInteractionMode
::ConvertIndexToAxialPlane( Vector3f index, int imageAxis )
{
  // Return x,y,z voxel indices in axial plane (numbering starting from 1)

  index[0] = std::floor( index[0] ) + 1;
  index[1] = std::floor( index[1] ) + 1;
  index[2] = std::floor( index[2] ) + 1;
  
  Vector3ui iSize = m_ParentUI->GetDriver()->GetCurrentImageData()->GetVolumeExtents();
  
  Vector3f axialIndex;
  axialIndex[0] = index[0];
  axialIndex[1] = index[1];
  axialIndex[2] = index[2];
 
  switch ( imageAxis )
  {
  case 2:  // Axial plane
    index[2] = iSize[2] - index[2] + 1;
    axialIndex = index;
    break;
  case 0: // Sagittal plane
    index[0] = iSize[1] - index[0] + 1;
    index[2] = iSize[0] - index[2] + 1;
    axialIndex[0] = index[2];
    axialIndex[1] = index[0];
    axialIndex[2] = index[1];
    break;
  case 1: // Coronal plane
    axialIndex[0] = index[0];
    axialIndex[1] = index[2];
    axialIndex[2] = index[1];
    break;
  }
