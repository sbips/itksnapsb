...
::SaveImage(const char *FileName, Registry &folder, itk::Image<TPixel,3> *image)
{
  ...
  Following ensures that:
   -values are correct when segmentation image is saved as analyze.
   -greyscale and segmentation images are saved using their native datatype
  */
  typedef  itk::Image< TPixel, 3 > ImageType;
  typedef itk::Image< float, 3 >   OutputImageType;

  // 1.  Create empty floating point image 
  typename OutputImageType::Pointer output = OutputImageType::New();
  output->SetRegions(image->GetLargestPossibleRegion().GetSize());
  output->SetSpacing(image->GetSpacing());
  output->SetDirection(image->GetDirection());
  output->Allocate();
  
  typename itk::ImageRegionIterator< ImageType >       imageIt(  image,  image->GetLargestPossibleRegion() );
  typename itk::ImageRegionIterator< OutputImageType > outputIt( output, output->GetLargestPossibleRegion() );

  // 2.  Copy values from image to empty floating point image 
  //      (necessary because AnalyzeImageIO only supports RGB<unsigned char> and ITKsnap uses
  //       RGB<short> and rounding errors result for segmentation images saved in analyze format)
  for ( imageIt.GoToBegin(), outputIt.GoToBegin(); !imageIt.IsAtEnd(); ++imageIt, ++outputIt )
  {
    itk::RGBPixel< short int > pixelRGB = imageIt.Get();
    float pixelGrey = 0.30 * pixelRGB.GetRed()  +  0.59 * pixelRGB.GetGreen()  +  0.11 * pixelRGB.GetBlue();
    outputIt.Set( pixelGrey );
  }

  // 3.  Save image in its native datatype
  std::string pixtype = this->GetComponentTypeAsStringInNativeImage();

  if ( pixtype == "unsigned_char" )
    this->WriteFloatAs< unsigned char >(FileName, output.GetPointer() );
  if ( pixtype == "char" )
    this->WriteFloatAs< char >(FileName, output.GetPointer() );
  if ( pixtype == "unsigned_short" )
    this->WriteFloatAs< unsigned short >(FileName, output.GetPointer() );
  if ( pixtype == "short" )
    this->WriteFloatAs< short >(FileName, output.GetPointer() );
  if ( pixtype == "unsigned_int" )
    this->WriteFloatAs< unsigned int >(FileName, output.GetPointer() );
  if ( pixtype == "int" )
    this->WriteFloatAs< int >(FileName, output.GetPointer() );
  if ( pixtype == "unsigned_long" )
    this->WriteFloatAs< unsigned long>(FileName, output.GetPointer() );
  if ( pixtype == "long" )
    this->WriteFloatAs< long >(FileName, output.GetPointer() );
  if ( pixtype == "float" )
    this->WriteFloatAs< float >(FileName, output.GetPointer() );
  if ( pixtype == "double" )
    this->WriteFloatAs< double >(FileName, output.GetPointer() );
  if ( pixtype == "unknown" )
    this->WriteFloatAs< unsigned char >(FileName, output.GetPointer() );
...
void
GuidedNativeImageIO
::WriteFloatAs(const char *FileName, itk::Image<float,3> *image)
{
  // Convert floating point data to desired output datatype
  // (No checking performed to ensure valid conversion).
