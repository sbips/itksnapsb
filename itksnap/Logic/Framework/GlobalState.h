...
enum ToolbarModeType 
{
  ...
  FLOODFILL_MODE,
  SABRE_MODE,
  LANDMARK_PAINTBRUSH_MODE,
  LANDMARK_LINE_DRAWING_MODE,
  ALIGNMENT_MODE,
  SABRE3D_MODE
};
...
enum LandmarkType
{
   ...
   PC,
   RPRON,
   LandmarkTypeMIN = CLEAR,
   LandmarkTypeMAX = RPRON
};
...
public:
  ...
  /** Get the current annotation settings */
  irisGetMacro(Landmarks, Landmarks*);

  /** Set the current annotation settings */
  irisSetMacro(Landmarks, Landmarks*);
  ...
  irisSetMacro(FloodFillLimitImage, FloodFillLimitImage*);
  irisGetMacro(FloodFillLimitImage, FloodFillLimitImage*);

private:

  FloodFillLimitImage* m_FloodFillLimitImage;
